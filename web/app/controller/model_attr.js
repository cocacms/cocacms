...
class ModelAttrController extends Controller {
  ...
  async validate() {
    const body = await this.ctx.validate({
      key: [{ required: true, message: '请设置字段关键字' }, { pattern: this.ctx.re.name, message: '字段关键字只能是字母、数字、_' }],
    ...
    await this.ctx.validate({
      model: [{ required: true, message: '请设置模型id' }, { pattern: this.ctx.re.intStr, message: '模型id只能是数字' }],
    }, this.ctx.params);
      ...
      optionsArray: [
        {
          ...
          defaultField: {
            type: 'object',
            fields: {
              value: { pattern: this.ctx.re.name, message: '选项参数的key只能是字母、数字、_' },
            },
          },
        },
      ],
  ...
  async update() {
    const body = await this.validate();
    await this.service.modelAttr.model(this.ctx.params.model);
    this.ctx.body = await this.service.modelAttr.update(this.ctx.params.id, body);
  }
  ...
  async adjustIndex() {
    ...
    await this.service.modelAttr.model(this.ctx.params.model);
    this.ctx.body = await this.service.modelAttr.adjustIndex(body.keys, body.keyType);
  }
}
