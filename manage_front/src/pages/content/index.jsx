...
class ContentList extends Component {
  ...
  renderContent = (current, attrs, rules, indexs) => {
    ...
    //1列表页 2单页 3表单页
    if (current.type === 1) {
      return <TablePage current={current} attrs={attrs} rules={rules} indexs={indexs} />
    }
    ...
    if (current.type === -1) {
      return <Alert message="暂无可操作内容，原因：栏目未绑定模型或模型不存在" type="warning" showIcon />
    }
          ...
          <Col xs={0} xl={4} className={styles.category}>
            {
              ...
              <Tree
                showLine
                defaultExpandAll
                onSelect={(value) => {this.onChangeCategory(value[0])}}
              >
