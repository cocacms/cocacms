...
import {
  Table, Form, Row, Col, Button, Input, Modal, Spin, Checkbox
} from 'antd';
        ...
        <Form>
          ...
          >
            {getFieldDecorator('account', {
              ...
              rules: [{ required: action === 'add', message: '请输入登录邮箱' }, { pattern: re.mail, message: '请输入正确的邮箱格式'}],
            })(
          ...
          >
            {getFieldDecorator('password', {
              initialValue: '',
              rules: [{ required: action === 'add', message: '请输入密码' }, { type: 'string', min: 8, message: '请输入8位以上'}],
            })(
              <Input />
            )}
          </Form.Item>
...
class RoleEdit extends Component {
  ...
  render() {
    const { admin: { roles = []}, opened, close, loading, rids = [] } = this.props;
    return (
      <Modal
        title="编辑角色"
        visible={opened}
          ...
          <Row>
            {
              roles.map(i => (
              <Col span={8} key={i.id}>
                <Checkbox value={i.id} onChange={this.onOk} defaultChecked={rids.includes(i.id)} >{i.name}</Checkbox>
              </Col>
              ))
            }
...
class AdminPage extends Component {
  ...
  openRoleModal = (uid, rids) => {
    this.setState({ roleEdit: { opened: true, uid, rids, key: Math.random() }})
  }
  ...
  getColumns = () => ([
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <Action
            can={{ edit: 'PUT@/api/admin/:id', delete: 'DELETE@/api/admin/:id'}}
            delete={() => { this.delete(record.id) }}
            edit={() => { this.openModal('edit', record) }}
          >
            <a onClick={() => { this.openRoleModal(record.id, record.roles.map(i => i.id )) }}>角色管理</a>
          </Action>
        )
  ...
  render() {
    const { loading, admin: { list: { data = [], page: current = 1, total = 0, pageSize = 20} = {}} } = this.props;
    return (
