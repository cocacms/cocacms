...
import {
  Table, Form, Row, Col, Button, Input, Modal, Select
} from 'antd';
...
class Edit extends Component {
  ...
  render() {
    const { action, opened, data = {}, close, form: { getFieldDecorator }, models = [] } = this.props;
          ...
          >
            {getFieldDecorator('model_id', {
              ...
              <Select placeholder="请选择">
                {
                  models.filter(i => i.type === 1).map(i => (<Select.Option key={`model_id_${i.id}`} value={i.id}>{i.name}</Select.Option>))
                }
              </Select>
            )}
          </Form.Item>
...
class FormPage extends Component {
  ...
  render() {
    const { loading, mform: { list = [], models = [] } } = this.props;
    return (
