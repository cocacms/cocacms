      ...
      columns.push({
        ...
        render: (text, record) => {
          ...
          if (['img'].includes(attr.type) && Array.isArray(text)) {
            return text.map(i => (<img key={`img_${i}`} src={i} alt="" style={{ maxWidth: 40, maxHeight: 40, marginRight: 5 }} />))
          }
          ...
          if (['time', 'date', 'datetime'].includes(attr.type)) {
            ...
            return moment(text, [moment.ISO_8601, 'YYYY-MM-DD HH:mm:ss', 'YYYY-MM-DD', 'HH:mm:ss']).format(formats[attr.type])
          }
